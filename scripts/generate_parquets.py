...
base_path = PROJECT_ROOT / settings.LOCAL_DATA_DIR.strip("./")  # Use "data" part of "./data"
...
securities = list(map(str, range(1000)))  # 1000 unique security identifiers (SEC_0 to SEC_999)
...
for data_field_identifier in data_field_identifiers:
    ...
    data = np.random.uniform(
        low=1,
        high=100,
        size=(len(dates), len(securities))
    )

    data_df = pd.DataFrame(
        data,
        index=dates,
        columns=[f"SEC_{s}" for s in securities]
    )
